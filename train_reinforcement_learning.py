...
def run(cli):
    ...
    # Uncomment if you desire to load a checkpoint
    ...
    for episode in range(num_episodes):
        init_state = cli.get_state()  # Assume get_state returns the initial state
        filtered_state = get_input_actor(init_state)
        filtered_state = torch.Tensor(filtered_state).unsqueeze(0).to(device)
        ...
        for timestamp in range(max_timesteps):
            ...
            if min_distance_paddle_ball(init_state) < 0.2 and not action_done:
                logger.info("Action done")
                action = agent.calc_action(filtered_state, action_noise=ou_noise)
            ...
            next_state = cli.get_state()  # Get the updated state from the environment
            filtered_next_state = get_input_actor(next_state)
            filtered_next_state = torch.Tensor(filtered_next_state).unsqueeze(0).to(device)

            if robot_paddle_touched_ball(next_state) and action_done and not push_done:
                ...
                memory.push(filtered_state, action, done_tensor, filtered_next_state, reward)
                push_done = True
        ...
        logger.info('Episode %d/%d, Reward: %f\n', episode + 1, num_episodes, episode_reward)
